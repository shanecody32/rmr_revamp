...
interface StaffAdvancedSearchDrawerProps {
    open: boolean;
    onClose: () => void;
...
export default function StaffAdvancedSearchDrawer({
                                                       open,
                                                       onClose,
                                                       onSearch,
                                                       filterType,
                                                       onFilterTypeChange,
                                                       filters
                                                   }: StaffAdvancedSearchDrawerProps) {
    const [form] = Form.useForm();
    const [loading, setLoading] = useState(false);
    const mounted = useRef(true);
    const [stationOptions, setStationOptions] = useState<{value: number; label: string}[]>([]);
    ...
    const handleStationSearch = async (searchValue: string) => {
        ...
        try {
            ...
            const options = result.data.map(station => ({
                value: station.id,
                label: station.name || `Station ${station.id}`,
            }));
            if (mounted.current) {
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Card title="Radio Station" size="small">
                    <Form.Item name="radio_station_id" label="Station">
                        <Select
                            showSearch
                            allowClear
                            ...
                            notFoundContent={stationSearchLoading ? 'Searching...' : 'Type to search stations'}
                        />
                    </Form.Item>
                </Card>
            </Form>
