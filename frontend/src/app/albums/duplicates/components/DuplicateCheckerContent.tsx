...
export default function DuplicateCheckerContent() {
    ...
    const isRunning = isDuplicateScanRunning('albums');
    const scanProgressFromSSE = getScanProgress('albums');
    ...
    useEffect(() => {
        if (prevScanRunning.current && !isRunning) {
            message.success('Album scan completed');
            loadResults();
        }
    ...
    const loadScanState = async () => {
        try {
            setLoading(true);
            const state = await getScanState('albums');
            setScanState(state);
    ...
    const loadResults = useCallback(async (page = 1) => {
        try {
            ...
            const response = await getCandidatesGrouped('albums', {
                page,
    ...
    const handleStartScan = async () => {
        try {
            await startScan('albums', scanSettings);
            message.success('Album scan started');
        } catch (error: any) {
    ...
    const handleStopScan = async () => {
        try {
            await stopScan('albums');
            message.success('Album scan stopped');
            loadResults();
        } catch (error) {
    ...
    const handleClearCandidates = async (pendingOnly: boolean) => {
        try {
            const result = await clearCandidates('albums', pendingOnly);
    ...
    const handleDismiss = async (candidateId: number) => {
        try {
            await updateCandidateStatus('albums', candidateId, 'dismissed');
    ...
    const handleRestore = async (candidateId: number) => {
        try {
            await restoreCandidate('albums', candidateId);
                    ...
                    {record.matches.some(m => m.status === 'pending') && (
                        <Button
                            ...
                            onClick={() => {
                                ...
                                Promise.all(pendingIds.map(id => updateCandidateStatus('albums', id, 'dismissed')))
                                    .then(() => {
            ...
            >
                <Table
                    ...
                    pagination={{
                        ...
                        showTotal: (total) => `${total} albums with potential duplicates`,
                    }}
                />
            </Card>
