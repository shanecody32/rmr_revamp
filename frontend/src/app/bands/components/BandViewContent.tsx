...
function ImagePreview({src, alt, onClose}: ImagePreviewProps) {
    ...
    return (
        <div
            className="fixed inset-0 z-50 bg-black bg-opacity-75 flex items-center justify-center p-4 custom-image-preview"
            onClick={onClose}
        >
            <div className="relative max-w-7xl max-h-[90vh] w-full">
                <Image
                    ...
                    priority
                    onError={(e) => {
                        console.warn(`Image preview error loading: ${src}: ${e}`);
                        // Next/Image handles fallbacks differently, we'll use a fallback URL
                        // in the src prop if needed
                    }}
                />
                <Button
                    type="text"
    ...
    useEffect(() => {
        const loadBand = async () => {
            try {
                ...
                if (data.city && data.country_id && data.city_id) {
                    // Create a proper CityResponse object from the available data
                                        ...
                                        <div className="relative group overflow-hidden rounded-lg">
                                            ...
                                            <div
                                                ...
                                                <Button
                                                    type="primary"
                                                    icon={<ExpandOutlined/>}
                                                    onClick={() => setPreviewImage(getBandImageUrl(img))}
                                                >
                                                    View
                                                </Button>
                                            </div>
                                ...
                                <div>
                                    ...
                                    <Space wrap className="mt-2">
                                        {band.genres?.map(genre => (
                                            <Tag key={genre.id} color="blue" className="text-base py-1 px-3">
                                                {genre.name}
                                            </Tag>
                                        ))}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {band.images && band.images.length > 0 ? (
                                ...
                                <div className="col-span-full py-12 text-center">
                                    ...
                                    <div className="mt-4 text-gray-500">No images available for this band.</div>
                                    <Button
                                        type="primary"
                                        icon={<EditOutlined/>}
                                        className="mt-4"
                                        onClick={() => message.info('Image upload coming soon')}
                                    >
                                        Add Images
                                    </Button>
                                </div>
            ...
            <Card className="mb-6">
                <div className="flex flex-wrap items-start gap-6">
                    ...
                    <div
                        ...
                        {band.images && band.images.length > 0 ? (
                            <ResponsiveImage
                                src={getBandImageUrl(band.images[0])}
                                alt={band.name}
                                className="w-full h-full"
                                objectFit="cover"
                            />
