...
export default function DuplicateCheckerContent() {
    ...
    const isRunning = isDuplicateScanRunning('labels');
    const scanProgressFromSSE = getScanProgress('labels');
    ...
    useEffect(() => {
        if (prevScanRunning.current && !isRunning) {
            message.success('Label scan completed');
            loadResults();
        }
    ...
    const loadScanState = async () => {
        try {
            setLoading(true);
            const state = await getScanState('labels');
            setScanState(state);
    ...
    const loadResults = useCallback(async (page = 1) => {
        try {
            ...
            const response = await getCandidatesGrouped('labels', {
                page,
    ...
    const handleStartScan = async () => {
        try {
            await startScan('labels', scanSettings);
            message.success('Label scan started');
        } catch (error: any) {
    ...
    const handleStopScan = async () => {
        try {
            await stopScan('labels');
            message.success('Label scan stopped');
            loadResults();
        } catch (error) {
    ...
    const handleClearCandidates = async (pendingOnly: boolean) => {
        try {
            const result = await clearCandidates('labels', pendingOnly);
    ...
    const handleDismiss = async (candidateId: number) => {
        try {
            await updateCandidateStatus('labels', candidateId, 'dismissed');
    ...
    const handleRestore = async (candidateId: number) => {
        try {
            await restoreCandidate('labels', candidateId);
                    ...
                    {record.matches.some(m => m.status === 'pending') && (
                        <Button
                            ...
                            onClick={() => {
                                ...
                                Promise.all(pendingIds.map(id => updateCandidateStatus('labels', id, 'dismissed')))
                                    .then(() => {
    ...
    return (
        <div className="p-6">
            ...
            <Text type="secondary" className="mb-6 block">
                Scan the database for potential duplicate labels, or search manually
            </Text>
            ...
            >
                <Table
                    ...
                    pagination={{
                        ...
                        showTotal: (total) => `${total} labels with potential duplicates`,
                    }}
                />
            </Card>
