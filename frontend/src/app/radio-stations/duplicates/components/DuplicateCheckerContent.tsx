...
export default function DuplicateCheckerContent() {
    ...
    const isRunning = isDuplicateScanRunning('radio_stations');
    const scanProgressFromSSE = getScanProgress('radio_stations');
    ...
    useEffect(() => {
        if (prevScanRunning.current && !isRunning) {
            message.success('Radio station scan completed');
            loadResults();
        }
    ...
    const loadScanState = async () => {
        try {
            setLoading(true);
            const state = await getScanState('radio_stations');
            setScanState(state);
    ...
    const loadResults = useCallback(async (page = 1) => {
        try {
            ...
            const response = await getCandidatesGrouped('radio_stations', {
                page,
    ...
    const handleStartScan = async () => {
        try {
            await startScan('radio_stations', scanSettings);
            message.success('Radio station scan started');
        } catch (error: any) {
    ...
    const handleStopScan = async () => {
        try {
            await stopScan('radio_stations');
            message.success('Radio station scan stopped');
            loadResults();
        } catch (error) {
    ...
    const handleClearCandidates = async (pendingOnly: boolean) => {
        try {
            const result = await clearCandidates('radio_stations', pendingOnly);
    ...
    const handleDismiss = async (candidateId: number) => {
        try {
            await updateCandidateStatus('radio_stations', candidateId, 'dismissed');
    ...
    const handleRestore = async (candidateId: number) => {
        try {
            await restoreCandidate('radio_stations', candidateId);
                    ...
                    {record.matches.some(m => m.status === 'pending') && (
                        <Button
                            ...
                            onClick={() => {
                                ...
                                Promise.all(pendingIds.map(id => updateCandidateStatus('radio_stations', id, 'dismissed')))
                                    .then(() => {
    ...
    return (
        <div className="p-6">
            ...
            <Text type="secondary" className="mb-6 block">
                Scan the database for potential duplicate radio stations, or search manually
            </Text>
            ...
            >
                <Table
                    ...
                    pagination={{
                        ...
                        showTotal: (total) => `${total} radio stations with potential duplicates`,
                    }}
                />
            </Card>
