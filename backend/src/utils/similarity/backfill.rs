...
async fn backfill_band_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let bands = bands::Entity::find().all(db).await?;
    let total = bands.len() as i32;
    ...
    for (i, band) in bands.into_iter().enumerate() {
        ...
        if (i + 1) % 10 == 0 || (i + 1) as i32 == total {
            update_progress(progress, "Bands", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
        }
    }
...
async fn backfill_album_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let albums = albums::Entity::find().all(db).await?;
    ...
    for (i, album) in albums.into_iter().enumerate() {
        ...
        if name.is_empty() { 
            if (i + 1) % 10 == 0 || (i + 1) as i32 == total {
                update_progress(progress, "Albums", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
            }
            continue; 
        }
...
async fn backfill_song_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let songs = songs::Entity::find().all(db).await?;
    ...
    for (i, song) in songs.into_iter().enumerate() {
        ...
        if name.is_empty() { 
            if (i + 1) % 10 == 0 || (i + 1) as i32 == total {
                update_progress(progress, "Songs", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
            }
            continue; 
        }
...
async fn backfill_label_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let labels = labels::Entity::find().all(db).await?;
    ...
    for (i, label) in labels.into_iter().enumerate() {
        ...
        if name.is_empty() { 
            if (i + 1) % 10 == 0 || (i + 1) as i32 == total {
                update_progress(progress, "Labels", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
            }
            continue; 
        }
...
async fn backfill_state_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let states = states::Entity::find().all(db).await?;
    ...
    for (i, state) in states.into_iter().enumerate() {
        ...
        if name.is_empty() { 
            if (i + 1) % 10 == 0 || (i + 1) as i32 == total {
                update_progress(progress, "States", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
            }
            continue; 
        }
...
async fn backfill_city_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let cities = cities::Entity::find().all(db).await?;
    let total = cities.len() as i32;
    ...
    for (i, city) in cities.into_iter().enumerate() {
        ...
        if name.is_empty() { 
            if (i + 1) % 25 == 0 || (i + 1) as i32 == total {
                update_progress(progress, "Cities", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
            }
            continue; 
        }
...
async fn backfill_postal_code_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let pcs = postal_codes::Entity::find().all(db).await?;
    let total = pcs.len() as i32;
    ...
    for (i, pc) in pcs.into_iter().enumerate() {
        ...
        if name.is_empty() { 
            if (i + 1) % 25 == 0 || (i + 1) as i32 == total {
                update_progress(progress, "Postal Codes", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
            }
            continue; 
        }
...
async fn backfill_radio_station_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let stations = radio_stations::Entity::find().all(db).await?;
    ...
    for (i, station) in stations.into_iter().enumerate() {
        ...
        if name.is_empty() { 
            if (i + 1) % 10 == 0 || (i + 1) as i32 == total {
                update_progress(progress, "Radio Stations", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
            }
            continue; 
        }
...
async fn backfill_staff_member_aliases(db: &DatabaseConnection, progress: &Arc<RwLock<BackfillJobState>>) -> Result<(), DbErr> {
    let members = staff_members::Entity::find().all(db).await?;
    ...
    for (i, member) in members.into_iter().enumerate() {
        ...
        if name.is_empty() { 
            if (i + 1) % 25 == 0 || (i + 1) as i32 == total {
                update_progress(progress, "Staff Members", (i + 1) as i32, total, (i + 1) as i32 == total, None).await;
            }
            continue; 
        }
